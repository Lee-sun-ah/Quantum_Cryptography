import random
from alice1 import Alice
from bob1 import Bob
from eve1 import Eve
...
for i in range(0,len(num_bits)) :
    ...
    while(eavesdropping_rate<=1):            
        ...
        qc_polarization=alice.measure_polarization() #alice가 비트와 basis로 측정한 polaraization
        eve.receive_p(qc_polarization)#도청자가없는 부분은 alice의 polarization으로 채운다.(원래 상태)
            
        eve.create_basis(num_bits[i]) #eve가 basis를 랜덤으로 선택한다.
        ...
        bob.create_basis(num_bits[i]) #bob이 basis를 랜덤으로 선택한다.
        bob.measure_bits(qc_polarization) #basis와 polarization으로 비트 측정
            
        basis=alice.send_basis() #alice가 고전채널로 basis를 보냄
        bits=alice.send_bits() #alice가 bit보냄
                    
        QBER.append(bob.measure_real_bits(basis,bits)) #alice랑 같은 basis인지 확인해 실제 bit 보관
